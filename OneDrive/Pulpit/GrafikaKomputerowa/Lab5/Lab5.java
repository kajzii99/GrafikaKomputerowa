...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
	...
	private void draw(GL2 gl2) {
		...
		switch (objectNumber){
			case 1:
				gl2.glClear(GL.GL_COLOR_BUFFER_BIT | GL.GL_DEPTH_BUFFER_BIT); // czysczenie bufora koloru
				gl2.glColor3d(0, 1, 0    ); //ustawianie koloru
				gl2.glBegin(GL.GL_LINE_STRIP); //rysowanie liniami

				float r = 0.0f, z = 0f;
				for(int i = 0; i < 19*360; i++) //obliczanie pkt spirali
				{
					r= (float) ((float) 0.5f*Math.exp(0.01f * (float) i * (Math.PI / 180f)));
					gl2.glVertex3f((float)(r*Math.cos((float) i * (Math.PI / 180f))),
							-z,
							(float)(r * Math.sin((float) i * (Math.PI / 180f))));
					z+=0.001f;
				}

				gl2.glEnd(); //koniec defininiowania
				break;

			case 2:
				gl2.glClear(GL.GL_COLOR_BUFFER_BIT | GL.GL_DEPTH_BUFFER_BIT); // czysczenie bufora koloru
				gl2.glColor3d(0, 1, 0    ); //ustawianie koloru
				gl2.glBegin(GL.GL_TRIANGLE_FAN); //rysowanie za pomoca wachlarza trojkatow
